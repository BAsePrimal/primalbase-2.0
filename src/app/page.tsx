...
export default function HomePage() {
  ...
  async function fetchUserData() {
    try {
      ...
      // 2. Buscar progresso da Jornada direto da nuvem (jornada_logs)
      const { data: jornadaData, error: jornadaError } = await supabase
        .from('jornada_logs')
        .select('day_number')
        .eq('user_id', session.user.id)
        .eq('status', 'concluido'); // Conta apenas os concluídos

      if (jornadaError) {
        console.error('Erro ao buscar jornada:', jornadaError);
      } else if (jornadaData) {
        // Se ele tem 5 linhas salvas lá, significa que ele está no dia 5!
        setDiasFeitos(jornadaData.length);
      }
